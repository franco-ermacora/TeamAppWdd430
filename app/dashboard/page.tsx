...
import { deleteCollectionCard } from '@/lib/actions';
...
export default async function DashboardPage(props: {
    ...
    return (
      <main className="max-w-4xl mx-auto px-6 py-20 text-center text-white space-y-6">
        ...
        <p className="text-zinc-400 text-sm">
          You need to be signed in to view and manage your personal card collection.
        </p>
        <div>
  ...
  const [totalItems, cards] = await Promise.all([
    prisma.collectionCard.count({ where: { userId: user.id } }),
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-12 space-y-8 text-white">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-800 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-violet-500">My Collection</h1>
          <p className="text-sm text-zinc-400 mt-1">Total cards in vault: {totalItems}</p>
        </div>

        <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
          <Form action="/dashboard" className="w-full sm:w-72">
            <input
              type="text"
              name="query"
              defaultValue={query}
      ...
      {cards.length === 0 ? (
        <div className="text-center py-20 text-zinc-500 border border-dashed border-zinc-800 rounded-xl space-y-3">
          ...
          <Link href="/database" className="inline-block text-xs text-violet-400 hover:underline">
            Explore the Database to add some &rarr;
          </Link>
        </div>
      ) : (
              ...
              >
                <div>
                  ...
                  <div className="bg-amber-100/90 text-zinc-900 border border-amber-700 p-2 rounded text-[11px] leading-tight min-h-[50px] mb-3 italic">
                    A powerful collectible card stored safely in your vault inventory.
                  </div>
                </div>

                <div className="pt-2 border-t border-amber-700/50">
                  <form action={deleteCollectionCard.bind(null, card.id)}>
                    <button
                      type="submit"
